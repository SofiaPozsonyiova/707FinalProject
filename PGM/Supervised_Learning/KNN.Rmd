...
load("/Users/sdm98/Documents/GitHub/Private707/data/Balanced/FinalValidation_BalancedUSE.RData")
...
x_balanced_test <- sapply(Test_Balanced_All %>% select(-SelfPerceivedHealth), as.numeric)
x_balanced_test12 <- sapply(Test_Balanced_sub12 %>% select(-SelfPerceivedHealth),as.numeric)
x_balanced_test5 <- sapply(Test_Balanced_sub5 %>% select(-SelfPerceivedHealth),as.numeric)

y_train <- sapply(Train_All %>% select(SelfPerceivedHealth), as.numeric)
y_test <- sapply(Test %>% select(SelfPerceivedHealth),as.numeric)
y_balanced_test <- sapply(Test_Balanced %>% select(SelfPerceivedHealth),as.numeric)
...
## Test Models on Unbalanced Data

### All Model
```{r}
#get sensitivity/specificity
pred_all <- predict(fit_all, x_test)

CrossTable(pred_all, y_test, prop.chisq = FALSE)

ctab <- table(pred_all,y_test)
...
```{r}
#get sensitivity/specificity
pred_12 <- predict(fit_12, x_test12)

CrossTable(pred_12, y_test, prop.chisq = FALSE)

ctab12 <- table(pred_12,y_test)
...
```{r}
#get sensitivity/specificity
pred_5 <- predict(fit_5, x_test5)

CrossTable(pred_5, y_test, prop.chisq = FALSE)

ctab5 <- table(pred_5,y_test)
...
## Unabalanced Test Set Results
```{r}
data.frame(model = c('All','Top 12','Top 5'),accuracy = c(acc, acc12, acc5), sensitivity = c(sens, sens12, sens5), specificity = c(spec, spec12, spec5), precision = c(prec, prec12, prec5))
```


## Test Models on Balanced Data

### All Model
```{r}
#get sensitivity/specificity
pred_all_balanced <- predict(fit_all, x_balanced_test)

CrossTable(pred_all_balanced, y_balanced_test, prop.chisq = FALSE)

ctab <- table(pred_all_balanced,y_balanced_test)
...
```{r}
#get sensitivity/specificity
pred_12_balanced <- predict(fit_12, x_balanced_test12)

CrossTable(pred_12_balanced, y_balanced_test, prop.chisq = FALSE)

ctab12 <- table(pred_12_balanced,y_balanced_test)
...
```{r}
#get sensitivity/specificity
pred_5_balanced <- predict(fit_5, x_balanced_test5)

CrossTable(pred_5_balanced, y_balanced_test, prop.chisq = FALSE)

ctab5 <- table(pred_5_balanced,y_balanced_test)
