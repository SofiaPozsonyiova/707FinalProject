...
# Plot
#lda_plot_data2 <- cbind(Validation_Standardized, predict(lda_model)$x)

# ggplot(lda_plot_data, aes(LD1, SelfPerceivedHealth)) + geom_point(aes(color = SelfPerceivedHealth))

#ggplot(lda_plot_data, aes(x = 1:nrow(lda_plot_data), y = LD1)) + geom_point(aes(color=SelfPerceivedHealth))
```
