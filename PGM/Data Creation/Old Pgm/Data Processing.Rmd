...
```{r fig.height=3,fig.width=3}
na_removed_dat <- na_removed_dat %>% mutate(SelfPerceivedHealth = ifelse(SelfPerceivedHealth == 1 |SelfPerceivedHealth == 2, 0,           ifelse(SelfPerceivedHealth == 3 |SelfPerceivedHealth == 4 | SelfPerceivedHealth == 5,1,NA))) %>% mutate(SelfPerceivedHealth = as.factor(SelfPerceivedHealth)) 


levels(na_removed_dat$SelfPerceivedHealth)
...
t <- glm(SelfPerceivedHealth ~ as.numeric(P49f) + as.numeric(P49l) + as.numeric(P26a) + as.numeric(P45) + as.numeric(P37), family = binomial, data = na_removed_dat)
exp(coef(t))
```
...
save(validation_test,file = "/Users/sofiapozsonyiova/Documents/GitHub/Private707/data/ValidationTest.RData")
