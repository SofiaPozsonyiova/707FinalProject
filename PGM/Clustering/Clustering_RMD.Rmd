...
sum(abs(colMeans(miss) - colMeans(same)) > 0.2)

data.frame(diff = abs(colMeans(miss) - colMeans(same))) %>% filter(diff > 0.2)
 
#learn about misclassifies for each group
...
sum(abs(colMeans(missgood) - colMeans(samegood)) > 1)

data.frame(diff = abs(colMeans(missgood) - colMeans(samegood))) %>% filter(diff > 1)
```
...
```{r}
#Good Self-Perceived Health: Those who were classified correctly were more likely to respond 'No' to P34, 'Do you have any long-term mental health, behavioral or emotional problems? Long-term means lasting 6 months or more.'

P34_missgood <-  missgood %>% group_by(P34) %>% summarise(n=n())%>%
  ggplot(., aes(x = P34, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "Misclassified Responses to P34 where SelfPerceivedHealth = 1")

P34_samegood <-  samegood %>% group_by(P34) %>% summarise(n=n())%>%
  ggplot(., aes(x = P34, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "Correctly Classified Responses to P34 where SelfPerceivedHealth = 1")

grid.arrange(P34_missgood, P34_samegood, nrow = 1, ncol = 2)

#Poor Self-Perceived Health: Those who were classified correctly were more likely to respond 'Yes' to P34, 'Do you have any long-term mental health, behavioral or emotional problems? Long-term means lasting 6 months or more.'

P34_misspoor <-  misspoor %>% group_by(P34) %>% summarise(n=n())%>%
  ggplot(., aes(x = P34, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "Misclassified Responses to P34 where SelfPerceivedHealth = 2")

P34_samepoor <-  samepoor %>% group_by(P34) %>% summarise(n=n())%>%
  ggplot(., aes(x = P34, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "Correctly Classified Responses to P34 where SelfPerceivedHealth = 2")

grid.arrange(P34_misspoor, P34_samepoor, nrow = 1, ncol = 2)
