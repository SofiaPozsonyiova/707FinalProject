...
### TAKING ONLY SAMPLE
samp <- na_removed_dat %>% group_by(SelfPerceivedHealth) %>% sample_frac(.01)

#Standardize
#remove outcome (SelfPerceivedHealth) and standardize all other variables
full_standardized <- samp %>% subset(select = -c(SelfPerceivedHealth, P28)) %>% mutate_all(~(scale(.) %>% as.vector))
```
