...
```{r}
setwd("/Users/sofiapozsonyiova/Documents/GitHub/707FinalProject")
...
na_removed_dat[,col_names] <- lapply(na_removed_dat[,col_names] , as.numeric)

#Standardize
#remove outcome (SelfPerceivedHealth) and standardize all other variables
full_standardized <- na_removed_dat %>% subset(select = -c(SelfPerceivedHealth, P28)) %>% mutate_all(~(scale(.) %>% as.vector))
```
...
table(phys_health_clus$clus[,1], na_removed_dat$FiveFV)

#Compare with Diab/Prediab
table(phys_health_clus$clus[,1], na_removed_dat$Diab_Prediab)

#Compare with long term disability
table(phys_health_clus$clus[,1], na_removed_dat$P33)

#No clear cluster relationship for good/bad physical health


#compare with self-perceived health 1 = good 2 = bad
table(phys_health_clus$clus[,1], na_removed_dat$SelfPerceivedHealth)

phys_crosstab <- CrossTable(phys_health_clus$clus[,1], na_removed_dat$SelfPerceivedHealth, prop.chisq = FALSE)
phys_prop <- 100*phys_crosstab$prop.col
  #16304 were misclassified (32.4%)
      ...
      phys_crosstab_5 <- CrossTable(phys_health_clus$clus[,1], na_removed_dat$P28, prop.chisq = FALSE)
...
save(physical_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/physical_clusters.RData")
```
...
table(psych_health_clus$clus[,1], na_removed_dat$Suicide)

#Compare with feeling nervous/anxious/on edge 1 = not at all, 4 = nearly every day
table(psych_health_clus$clus[,1], na_removed_dat$P50c)
...
table(psych_health_clus$clus[,1], na_removed_dat$SelfPerceivedHealth)
  #2 = good, 1 = bad?
  #13629 misclassified (27.11%)

      psych_crosstab <- CrossTable(psych_health_clus$clus[,1], na_removed_dat$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      psych_binary <- ggplot(psych_prop, aes(x = y, y = Freq, fill = x))+
        ...
        scale_fill_brewer(labels = c('Good Health','Bad Health'),palette = 'Paired')+ 
  theme(axis.text = element_text(size = 20), 
       ...
       ggsave("/Users/sofiapozsonyiova/Documents/psych_1.png") 
   
      psych_crosstab_5 <- CrossTable(psych_health_clus$clus[,1], na_removed_dat$P28, prop.chisq = FALSE)
      ...
      psych_5 <- ggplot(psych_prop_5, aes(x = y, y = Freq, fill = x))+
        ...
        coord_flip()+
        labs(x = "Self-Perceived Health", y = "Percentage",fill = "Cluster")+
        ggtitle(label = "Prop Psychological Health Cluster in Self-Perceived Health Groups")+ 
        scale_fill_brewer(labels = c('Good Health','Bad Health'),palette = 'Paired')+  
  theme(axis.text = element_text(size = 20), 
        axis.title = element_text(size = 24), 
        plot.title = element_text(size = 20),
       ...
       ggsave("/Users/sofiapozsonyiova/Documents/psych_5.png")
...
# save(psychological_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/psychological_clusters.RData")
...
#this may suggest cluster 2 = bad social support and cluster 1 = good social support
      ...
      social_crosstab_5 <- CrossTable(social_clus$clus[,1], na_removed_dat$P28, prop.chisq = FALSE)
...
save(social_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/social_clusters.RData")
...
table(school_clus$clus[,1], na_removed_dat$SelfPerceivedHealth)
  #17416 misclassified (34.6%)

      
      school_crosstab <- CrossTable(school_clus$clus[,1], na_removed_dat$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      school_crosstab_5 <- CrossTable(school_clus$clus[,1], na_removed_dat$P28, prop.chisq = FALSE)
...
save(school_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/school_clusters.RData")
```
...
table(demo_clus$clus[,1], na_removed_dat$SelfPerceivedHealth)
  #21560 misclassified (43%)



      
      demo_crosstab <- CrossTable(demo_clus$clus[,1], na_removed_dat$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      demo_crosstab_5 <- CrossTable(demo_clus$clus[,1], na_removed_dat$P28, prop.chisq = FALSE)
...
save(demo_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/demo_clusters.RData")
```
...
table(subset_clus$clus[,1], na_removed_dat$SelfPerceivedHealth)
  #7668 misclassified 

      subset_crosstab <- CrossTable(subset_clus$clus[,1], na_removed_dat$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      subset_crosstab_5 <- CrossTable(subset_clus$clus[,1], na_removed_dat$P28, prop.chisq = FALSE)
...
poorind <- which(na_removed_dat$SelfPerceivedHealth == 2,)
poorself <- subset[poorind,]
poorself_clust <- (as.data.frame(subset_clus$clus[,1]))[poorind,]

misspoor <- poorself[na_removed_dat$SelfPerceivedHealth[poorind] != poorself_clust,]
samepoor <- poorself[na_removed_dat$SelfPerceivedHealth[poorind] == poorself_clust,]
...
goodind <- which(na_removed_dat$SelfPerceivedHealth == 1,)
goodself <- subset[goodind,]
goodself_clust <- (as.data.frame(subset_clus$clus[,1]))[goodind,]

missgood <- goodself[na_removed_dat$SelfPerceivedHealth[goodind] != goodself_clust,]
samegood <- goodself[na_removed_dat$SelfPerceivedHealth[goodind] == goodself_clust,]
...
table(subset_clus5$cluster, na_removed_dat$SelfPerceivedHealth)
fviz_cluster(subset_clus5, psych_health, geom = 'point', main = 'Clusters with combination of categories')

      subset_crosstab5 <- CrossTable(subset_clus5$cluster, na_removed_dat$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      subset_crosstab_55 <- CrossTable(subset_clus5$cluster, na_removed_dat$P28, prop.chisq = FALSE)
