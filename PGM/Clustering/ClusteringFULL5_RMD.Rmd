...
school_clus <- kmeans(school, centers = 2, nstart = 10)
table(school_clus$cluster, Train$SelfPerceivedHealth)
  #9746 misclassified


fviz_cluster(school_clus, school, geom = 'point', main = 'Clusters with School Variables')

      
      school_crosstab <- CrossTable(school_clus$cluster, Train$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      school_crosstab_5 <- CrossTable(school_clus$cluster, Train$P28, prop.chisq = FALSE)
...
demo_clus <- kmeans(demo, centers = 2, nstart = 10)
table(demo_clus$cluster, Train$SelfPerceivedHealth)
  #12156 misclassified


fviz_cluster(demo_clus, demo, geom = 'point', main = 'Clusters with Demographics')

      
      demo_crosstab <- CrossTable(demo_clus$cluster, Train$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      demo_crosstab_5 <- CrossTable(demo_clus$cluster, Train$P28, prop.chisq = FALSE)
...
sum(abs(colMeans(missgood) - colMeans(samegood)) > 1.0353)

data.frame(diff = abs(colMeans(missgood) - colMeans(samegood))) %>% filter(diff > 1.03533)
```
