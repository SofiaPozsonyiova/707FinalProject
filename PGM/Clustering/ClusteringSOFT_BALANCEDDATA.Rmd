...
load("/Users/sdm98/Documents/GitHub/Private707/data/Balanced/SubsetDat.RData")

#change back to numeric
col_names <- names(CLEANDAT)
CLEANDAT[,col_names] <- lapply(CLEANDAT[,col_names] , as.numeric)

#Standardize
#remove outcome (SelfPerceivedHealth) and standardize all other variables
full_standardized <- CLEANDAT %>% subset(select = -c(SelfPerceivedHealth, P28)) %>% mutate_all(~(scale(.) %>% as.vector))
...
table(phys_health_clus$clus[,1], CLEANDAT$FiveFV)

#Compare with Diab/Prediab
table(phys_health_clus$clus[,1], CLEANDAT$Diab_Prediab)

#Compare with long term disability
table(phys_health_clus$clus[,1], CLEANDAT$P33)

#No clear cluster relationship for good/bad physical health


#compare with self-perceived health 1 = good 2 = bad
table(phys_health_clus$clus[,1], CLEANDAT$SelfPerceivedHealth)

phys_crosstab <- CrossTable(phys_health_clus$clus[,1], CLEANDAT$SelfPerceivedHealth, prop.chisq = FALSE)
phys_prop <- 100*phys_crosstab$prop.col
  #16304 were misclassified (32.4%)
      ...
      phys_binary <- ggplot(phys_prop, aes(x = y, y = Freq, fill = x))+
        ...
        scale_fill_brewer(labels = c('Good Health','Bad Health'),palette = 'Paired', limits = c(2, 1))
      
      phys_crosstab_5 <- CrossTable(phys_health_clus$clus[,1], CLEANDAT$P28, prop.chisq = FALSE)
...
#physical_clusters <- as.data.frame(phys_health_clus$U)
#physical_clusters$subject <- rownames(physical_clusters)
#save(physical_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/Balanced/physical_clusters.RData")
```

```{r}
#cluster visualization
fviz_cluster(list(cluster = phys_health_clus$clus[,1],data = phys_health), phys_health, labelsize = 0, main = 'Cluster plot for Physical Health Variables')
```

```{r}
#cluster visualization
ggplot(phys_health, aes(x = as.factor(CLEANDAT$P28), y = P37, color = as.factor(phys_health_clus$clus[,1]))) +
        geom_point() + 
        labs(x = "Self-Perceived Health", y = "P37: Physical Activity",color = "Cluster")+
        ggtitle(label = "Cluster Visualization")+ 
        scale_fill_brewer(labels = c('Good Health','Bad Health'),palette = 'Paired', limits = c(2,1))
```
...
psych_health <- full_standardized %>%
...
table(psych_health_clus$clus[,1], CLEANDAT$Suicide)

#Compare with feeling nervous/anxious/on edge 1 = not at all, 4 = nearly every day
table(psych_health_clus$clus[,1], CLEANDAT$P50c)
...
table(psych_health_clus$clus[,1], CLEANDAT$SelfPerceivedHealth)
  #2 = good, 1 = bad?
  #13629 misclassified (27.11%)

      psych_crosstab <- CrossTable(psych_health_clus$clus[,1], CLEANDAT$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      psych_binary <- ggplot(psych_prop, aes(x = y, y = Freq, fill = x))+
        ...
        scale_fill_brewer(labels = c('Good Health','Bad Health'),palette = 'Paired', limits = c(1, 2))
      
      psych_crosstab_5 <- CrossTable(psych_health_clus$clus[,1], CLEANDAT$P28, prop.chisq = FALSE)
...
#psychological_clusters <- as.data.frame(psych_health_clus$U)
#psychological_clusters$subject <- rownames(psychological_clusters)
#save(psychological_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/Balanced/psychological_clusters.RData")

```

```{r}
#cluster visualization
fviz_cluster(list(cluster = psych_health_clus$clus[,1],data = psych_health), psych_health, labelsize = 0, main = 'Cluster plot for Psychological Health Variables')
```

```{r}
#cluster visualization
ggplot(psych_health, aes(y = as.factor(CLEANDAT$P28), x = P49f, color = as.factor(psych_health_clus$clus[,1]))) +
        geom_jitter() + 
        labs(y = "Self-Perceived Health", x = "P49f: I feel good about my future",color = "Cluster")+
        ggtitle(label = "Psychological Health Cluster Visualization")
```

```{r}
#Social Support Related
social <- full_standardized %>%
...
#this may suggest cluster 2 = bad social support and cluster 1 = good social support
      ...
      social_crosstab_5 <- CrossTable(social_clus$clus[,1], CLEANDAT$P28, prop.chisq = FALSE)
...
#social_clusters <- as.data.frame(social_clus$U)
#social_clusters$subject <- rownames(social_clusters)
#save(social_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/Balanced/social_clusters.RData")
...
table(school_clus$clus[,1], CLEANDAT$SelfPerceivedHealth)
  #17416 misclassified (34.6%)

      
      school_crosstab <- CrossTable(school_clus$clus[,1], CLEANDAT$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      school_crosstab_5 <- CrossTable(school_clus$clus[,1], CLEANDAT$P28, prop.chisq = FALSE)
...
#school_clusters <- as.data.frame(school_clus$U)
#school_clusters$subject <- rownames(school_clusters)
#save(school_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/Balanced/school_clusters.RData")
```

```{r}
#School Related
demo <- full_standardized %>%
...
table(demo_clus$clus[,1], CLEANDAT$SelfPerceivedHealth)
  #21560 misclassified (43%)



      
      demo_crosstab <- CrossTable(demo_clus$clus[,1], CLEANDAT$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      demo_crosstab_5 <- CrossTable(demo_clus$clus[,1], CLEANDAT$P28, prop.chisq = FALSE)
...
#demo_clusters <- as.data.frame(demo_clus$U)
#demo_clusters$subject <- rownames(demo_clusters)
#save(demo_clusters,file ="/Users/sdm98/Documents/GitHub/Private707/data/Balanced/demo_clusters.RData")
```
...
table(subset_clus$clus[,1], CLEANDAT$SelfPerceivedHealth)
  #7668 misclassified 

      subset_crosstab <- CrossTable(subset_clus$clus[,1], CLEANDAT$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      subset_binary <- ggplot(propcol, aes(x = y, y = Freq, fill = x))+
        ...
        coord_flip()+
        labs(x = "Self-Perceived Health", y = "Percentage",fill = "Cluster")+
        ggtitle(label = "Proportion of Each Combined Cluster in Self-Perceived Health Groups")+ 
        scale_fill_brewer(labels = c('Good Health','Bad Health'),palette = 'Paired', limits = c(2, 1))
      
      
      subset_crosstab_5 <- CrossTable(subset_clus$clus[,1], CLEANDAT$P28, prop.chisq = FALSE)
...
```{r}
#cluster visualization
fviz_cluster(list(cluster = subset_clus$clus[,1],data = subset), subset, labelsize = 0, main = 'Cluster plot for All Variables')
```
...
poorind <- which(CLEANDAT$SelfPerceivedHealth == 2,)
poorself <- subset[poorind,]
poorself_clust <- (as.data.frame(subset_clus$clus[,1]))[poorind,]

misspoor <- poorself[CLEANDAT$SelfPerceivedHealth[poorind] != poorself_clust,]
samepoor <- poorself[CLEANDAT$SelfPerceivedHealth[poorind] == poorself_clust,]
...
goodind <- which(CLEANDAT$SelfPerceivedHealth == 1,)
goodself <- subset[goodind,]
goodself_clust <- (as.data.frame(subset_clus$clus[,1]))[goodind,]

missgood <- goodself[CLEANDAT$SelfPerceivedHealth[goodind] != goodself_clust,]
samegood <- goodself[CLEANDAT$SelfPerceivedHealth[goodind] == goodself_clust,]
...
P47a_missgood <-  missgood %>% group_by(P47a) %>% summarise(n=n())%>%
  ...
  ggtitle(label = "P47a: Misclassified SPH = 1")

P47a_samegood <-  samegood %>% group_by(P47a) %>% summarise(n=n())%>%
  ggplot(., aes(x = P47a, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "P47a: Correct SPH = 1")
...
P47a_misspoor <-  misspoor %>% group_by(P47a) %>% summarise(n=n())%>%
  ggplot(., aes(x = P47a, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "P47a: Misclassified SPH = 2")

P47a_samepoor <-  samepoor %>% group_by(P47a) %>% summarise(n=n())%>%
  ggplot(., aes(x = P47a, y = n))+
  geom_col(stat = "identity")+
  ggtitle(label = "P47a: Correct SPH = 2")
...
table(subset_clus5$cluster, CLEANDAT$SelfPerceivedHealth)
fviz_cluster(subset_clus5, psych_health, geom = 'point', main = 'Clusters with combination of categories')

      subset_crosstab5 <- CrossTable(subset_clus5$cluster, CLEANDAT$SelfPerceivedHealth, prop.chisq = FALSE)
      ...
      subset_crosstab_55 <- CrossTable(subset_clus5$cluster, CLEANDAT$P28, prop.chisq = FALSE)
