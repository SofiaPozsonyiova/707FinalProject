...
# ifelse(P80c == 6 | P80d == 6 | P80e == 6 | P80f == 6 | P80g == 6 | P80i == 6,6,"ERROR"))))))) %>% dplyr::select(-c(P80c,P80d,P80e,P80f,P80g,P80i))
```
...
# check %>% group_by(P80d) %>% summarise(cnt = n()) %>% mutate(freq = round(cnt / sum(cnt), 3)) %>%  arrange(desc(freq))
# check %>% group_by(P80e) %>% summarise(cnt = n()) %>% mutate(freq = round(cnt / sum(cnt), 3)) %>%  arrange(desc(freq))
# check %>% group_by(P80f) %>% summarise(cnt = n()) %>% mutate(freq = round(cnt / sum(cnt), 3)) %>%  arrange(desc(freq))
# check %>% group_by(P80g) %>% summarise(cnt = n()) %>% mutate(freq = round(cnt / sum(cnt), 3)) %>%  arrange(desc(freq))
# check %>% group_by(P80i) %>% summarise(cnt = n()) %>% mutate(freq = round(cnt / sum(cnt), 3)) %>%  arrange(desc(freq))
# 
# 
# 
# na_removed_dat %>%
...
P29e	Have you had any of the following dental problems during the past 12 months? Missed one or more school days because of a dental problem
0 = No  
1 = Yes 
```{r}
# na_removed_dat <- na_removed_dat %>% mutate(DentalIssues = ifelse(P29a == 0 | P29b == 0|P29c == 0| P29d == 0|  P29e == 0,0,
# ifelse(P29a == 1| P29b == 1|P29c == 1| P29d == 1|  P29e == 1,1,NA))) %>% dplyr::select(-c(P29a:P29f))
```
