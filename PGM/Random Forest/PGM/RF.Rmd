...
var_impTop <- var_impAll %>% arrange(desc(importance)) %>% slice_head(n = 25)

## Create a plot of variable importance for the top 25 variables
var_impPlot <- var_impTop %>% 
  arrange(importance) %>%
  ggplot(aes(x=reorder(variables, importance), y=importance)) + 
  geom_bar(stat='identity', fill = "cornflowerblue") + 
  coord_flip() + 
  xlab('Variables') +
  labs(title='Random Forest Variable Importance (Top 25 predictors)') + ylab("Gini Impurity") + 
