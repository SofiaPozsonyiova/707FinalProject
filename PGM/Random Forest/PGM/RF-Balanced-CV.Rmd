...
## Print the accuracy
accuracy <- mean(y_hats5 == Train_sub4$SelfPerceivedHealth)*100
cat('Accuracy on testing data: ', round(accuracy, 2), '%',  sep='')
```

```{r}
# Confusion matrix 
conf_5 <- table(Train_sub4$SelfPerceivedHealth, y_hats5)
conf_5

confusionMatrix(table(Train_sub4$SelfPerceivedHealth, y_hats5))
```
...
predicted__test <- stats::predict(log_reg_all, Test)

# Run confusion matrix
caret::confusionMatrix(predicted__test, as.factor(Test$SelfPerceivedHealth))
```
...
```{r}
# Predict on train data
predicted__test12 <- stats::predict(log_reg_12, Test)

# Run confusion matrix
caret::confusionMatrix(predicted__test12, as.factor(Test$SelfPerceivedHealth))
```
...
```{r}
# Predict on train data
predicted__test5 <- stats::predict(log_reg_5, Test)

# Run confusion matrix
caret::confusionMatrix(predicted__test5, as.factor(Test$SelfPerceivedHealth))
```
